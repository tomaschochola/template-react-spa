...
DEVCONTAINER_PROJECT := template-react-spa-devcontainer
...
local_swarm development_swarm sit_swarm uat_swarm production_swarm:
	docker stack deploy -c ./docker-compose.yml -c ./docker-compose-swarm.yml --with-registry-auth --prune --detach=false --resolve-image=always "$${CI_PROJECT_PATH_SLUG:-template-react-spa}-$${APP_ENV}"
...
local_swarm_remove development_swarm_remove sit_swarm_remove uat_swarm_remove production_swarm_remove:
	docker stack rm "$${CI_PROJECT_PATH_SLUG:-template-react-spa}-$${APP_ENV}"
